...
**Symptom.** New Laravel app `demo-app-a` (panel **New Laravel App**) failed at migrate:
...
(peer-app-b / peer-app-c / demo-app-a Postgres on `shared_proxy_net`).
...
**Symptom.** Site Web SQLite backup copied `/apps/site-web/database/database.sqlite` on the host and reported success. The live file was `panel-site-web_database` (`/app/database/database.sqlite`). Host mtime froze at template switch; hashes differed a few hours later.
...
**Symptom.** Acme App OneDrive backup of `storage/app/public/attachments` showed
16,540 items and could not find a file the app had served (`permit.pdf` /
`aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.pdf`). The same path on the host
...
**Symptom.** Clicking Run on a backup (Acme files) succeeded, but no email
arrived. Mail **Test** already worked. The job is in notification group
“Nightly backups” with two other backups.
