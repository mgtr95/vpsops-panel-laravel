...
JOBS = [
    {"id": "pg", "name": "Zuc Uz PostgreSQL"},
    {"id": "files", "name": "Zuc Uz files"},
    {"id": "thumbs", "name": "Zuc Uz thumbnails"},
]
...
class GroupNotifyTests(unittest.IsolatedAsyncioTestCase):
    async def test_waiting_message_lists_unfinished_jobs(self):
        ...
        self.assertIn("Zuc Uz PostgreSQL", msg)
        self.assertIn("Zuc Uz thumbnails", msg)
        self.assertNotIn("Zuc Uz files", msg.split("waiting for:")[1])
    ...
    async def test_manual_job_uses_group_recipients(self):
        ...
        job = {"id": "files", "name": "Zuc Uz files", "notify": {"group_id": "grp1"}}
